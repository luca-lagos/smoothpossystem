    ...
    <div class="container-fluid mb-4 col-12">
        ...
        <div class="card shadow mb-4">
            <div class="card-header py-3 d-flex justify-content-between align-items-center">
                ...
                <a href="{{ route('suppliers.create') }}"><button type="button" class="btn btn-primary">CREAR UN
                        PROVEEDOR</button></a>
            </div>
            <div class="card-body">
                    ...
                    <table class="table table-bordered table-striped" id="dataTable" width="100%" cellspacing="0">
                        ...
                        <tbody>
                            @foreach ($suppliers as $supplier)
                                <tr>
                                    <td>{{ $supplier->people->social_reason }}</td>
                                    <td>{{ $supplier->people->person_type }}</td>
                                    <td>{{ $supplier->people->document_number }}</td>
                                    <td>{{ $supplier->people->document->document_type }}</td>
                                    <td>{{ $supplier->people->location }}</td>
                                    <td>
                                        @if ($supplier->people->status == 1)
                                    ...
                                    <td class="d-flex justify-content-center align-items-center">
                                        <div class="btn-group">
                                            <form action="{{ route('suppliers.edit', ['supplier' => $supplier]) }}">
                                                @csrf
                                            ...
                                            @if ($supplier->people->status == 1)
                                                <button class="btn btn-danger rounded px-4 ml-1" data-toggle="modal"
                                                    data-target="#confirmModal-{{ $supplier->id, $supplier->social_reason }}"><i
                                ...
                                <!-- Confirmation modal -->
                                <div class="modal fade" id="confirmModal-{{ $supplier->id}}" tabindex="-1"
                                    ...
                                    <div class="modal-dialog">
                                        @if ($supplier->people->status == 1)
                                            <div class="modal-content">
                                                ...
                                                <div class="modal-footer">
                                                    ...
                                                    <form
                                                        action="{{ route('suppliers.destroy', ['supplier' => $supplier->people->id]) }}"
                                                        method="post">
                                                        @method('DELETE')
                                                        @csrf
                                        ...
                                        @else
                                            <div class="modal-content">
                                                ...
                                                <div class="modal-body">
                                                    ¿Estás seguro de activar al proveedor
                                                    "{{ $supplier->people->social_reason }}"?
                                                </div>
                                                <div class="modal-footer">
                                                    <button type="button" class="btn btn-secondary"
